  ...
  activate(state) {
    return atom.commands.onWillDispatch(e  => {
      if (e.type === "core:paste") {
        ...
        const assetsDirPath = thefile.getParent().getPath()+"/"+atom.config.get('markdown-image-helper.directoryName')
        ...
        this.createDirectory(assetsDirPath, ()=> {
          return this.writePng(assetsDirPath+'/', filename, imgbuffer, ()=> {
            return this.insertUrl(atom.config.get('markdown-image-helper.directoryName')+`/${filename}`,editor)
          })
        })
